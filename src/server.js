import logger from '../components/logs.js'
...
if (args.iscluster == "cluster" && cluster.isPrimary) {
    ...
    cpus.map(() => {
     ...
     servidor = app.listen(process.env.PORT || 8080, () => { 
        datosLogin.logger.info(`Servidor conectado puerto info 8080 PID - ${process.pid}`)
    })
 }
